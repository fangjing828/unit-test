 ...
 */
public class ThreadPoolTransferContextByWrapRunnable {
    ...
    void execute()  {
        ...
        try {
            Future future1 = executor.submit(TtlRunnable.get(this::step1));
            Thread.sleep(ThreadLocalRandom.current().nextLong(1000));
            Future future2 = executor.submit(TtlRunnable.get(this::step2));
            Thread.sleep(ThreadLocalRandom.current().nextLong(1000));
            Future future3 = executor.submit(TtlRunnable.get(this::step3));
            future1.get();
            future2.get();
            future3.get();
    ...
    public static void main(String[] args) {
        new Thread(() -> new ThreadPoolTransferContextByWrapRunnable().execute()).start();
        new Thread(() -> new ThreadPoolTransferContextByWrapRunnable().execute()).start();
        new Thread(() -> new ThreadPoolTransferContextByWrapRunnable().execute()).start();
    }
}
